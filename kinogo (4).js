(function () {
    ...
    // Источник: kinogo.la — поиск, страница фильма, плеер ortified.ws / stravers.live
    // ============================================================
    ...
    Lampa.Lang.add({
        ...
        kinogo_region: { ru: 'Видео недоступно в вашем регионе', en: 'Video not available in your region', uk: 'Відео недоступне у вашому регіоні' },
        kinogo_proxy: { ru: 'Прокси Kinogo', en: 'Kinogo Proxy', uk: 'Проксі Kinogo' },
        kinogo_proxy_descr: { ru: 'CORS прокси (напр. https://corsproxy.io/? или свой)', en: 'CORS proxy', uk: 'CORS проксі' }
    });
    ...
    function KinogoComponent(object) {
        ...
        this.create = function () {
            ...
            console.log('Kinogo', 'search:', searchTitle);
        ...
        this.searchKinogo = function (title) {
            ...
            network.native(searchUrl, function (html) {
                console.log('Kinogo', 'search result, length:', (html || '').length);
                var resultLink = _this.parseSearch(html, title, active_card.year);

                if (resultLink) {
                    console.log('Kinogo', 'found:', resultLink);
                    _this.loadFilmPage(resultLink);
                } else {
                    console.log('Kinogo', 'no match found');
                    _this.activity.loader(false);
                    _this.emptyShow();
                    _this.activity.toggle();
                }
            }, function (a, c) {
                console.log('Kinogo', 'search error:', a, c);
                _this.activity.loader(false);
                _this.emptyShow();
                _this.activity.toggle();
            }, false, { dataType: 'text' });
        ...
        this.loadFilmPage = function (href) {
            ...
            network.native(filmUrl, function (html) {
                console.log('Kinogo', 'film page loaded, length:', (html || '').length);
                // Собираем ВСЕ iframe data-src URL
                var iframeUrls = [];
                var iframeRegex = /<iframe[^>]+data-src="([^"]+)"/g;
                var iframeMatch;
                while ((iframeMatch = iframeRegex.exec(html)) !== null) {
                    var url = decodeHTMLEntities(iframeMatch[1]);
                    if (url.indexOf('//') === 0) url = 'https:' + url;
                    // Пропускаем трейлеры
                    if (url.indexOf('/trailer/') !== -1) continue;
                    iframeUrls.push(url);
                }

                console.log('Kinogo', 'found iframes:', iframeUrls.length, iframeUrls);

                if (iframeUrls.length > 0) {
                    _this.tryPlayers(iframeUrls, 0);
                } else {
                    _this.activity.loader(false);
                    _this.emptyShow();
                    _this.activity.toggle();
                ...
                console.log('Kinogo', 'film page error:', a, c);
                _this.activity.loader(false);
                _this.emptyShow();
                _this.activity.toggle();
            }, false, { dataType: 'text' });
        ...
        // ---- Попробовать несколько плееров по очереди ----
        this.tryPlayers = function (urls, index) {
            var _this = this;
            if (index >= urls.length) {
                console.log('Kinogo', 'all players failed');
                _this.activity.loader(false);
                _this.emptyShow();
                _this.activity.toggle();
                return;
            ...
            var embedUrl = urls[index];
            console.log('Kinogo', 'trying player', index + 1, ':', embedUrl);

            _this.loadPlayer(embedUrl, function (success) {
                if (!success) {
                    // Пробуем следующий плеер
                    _this.tryPlayers(urls, index + 1);
                }
            });
        };

        // ---- Загрузка плеера ----
        this.loadPlayer = function (embedUrl, onResult) {
            var _this = this;
            var playerUrl = proxify(embedUrl);

            // ВАЖНО: НЕ добавляем Referer заголовок — он вызывает CORS preflight,
            // который падает на ortified.ws (нет Access-Control-Allow-Headers)
            network.timeout(15000);
            network.native(playerUrl, function (html) {
                console.log('Kinogo', 'player page loaded, length:', (html || '').length);

                // Проверяем региональную блокировку
                if (html && (html.indexOf('недоступно для вашего региона') !== -1 ||
                    html.indexOf('недоступен в вашем регионе') !== -1)) {
                    console.log('Kinogo', 'region blocked');
                ...
                if (fileMatch) {
                    ...
                    console.log('Kinogo', 'found file, encoded length:', fileEncode.length);
                    ...
                    if (m3u8Match) {
                        console.log('Kinogo', 'found direct m3u8');
                        _this.buildFromStream(m3u8Match[1]);
                        if (onResult) onResult(true);
                    } else {
                        console.log('Kinogo', 'no file or m3u8 found in player page');
                        if (onResult) onResult(false);
                    }
                }
            }, function (a, c) {
                console.log('Kinogo', 'player error:', a, c);
                if (onResult) onResult(false);
            }, false, { dataType: 'text' });
        };
        ...
        this.decodeAndBuild = function (fileEncode) {
            ...
            try {
                ...
                console.log('Kinogo', 'decode error:', e.message);
                _this.activity.loader(false);
                _this.emptyShow();
                _this.activity.toggle();
                return;
            ...
            console.log('Kinogo', 'decoded, type:', typeof result, Array.isArray(result) ? 'array(' + result.length + ')' : 'object');
        ...
        this.addItem = function (title, fileUrl, subtitles) {
            ...
            // НЕ проксируем m3u8 потоки — они обычно работают напрямую
            // Прокси нужен только для HTML-страниц
    ...
    Lampa.Template.add('settings_kinogo', '<div>' +
        '<div class="settings-param selector" data-type="input" data-name="kinogo_proxy" placeholder="https://corsproxy.io/?">' +
